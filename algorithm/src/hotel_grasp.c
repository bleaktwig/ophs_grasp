...
bool tour_grc(uint trips_n, uint hotels_n, uint pois_n, uint rcl,
    ...
    for (uint i = 0; i < trips_n - 1; ++i) {
        ...
        if (endadd_v(&tour[i], v, cndt_h.items[choice].idx, hotels_n, d_matrix, 0)) return 1;
        if (endadd_v(&tour[i+1], v, cndt_h.items[choice].idx, hotels_n, d_matrix, 1)) return 1;

        vervec_pop(&cndt_h, choice);
    }
    if (endadd_v(&tour[trips_n-1], v, 1, hotels_n, d_matrix, 0)) return 1;
    vervec_free(&cndt_h);
